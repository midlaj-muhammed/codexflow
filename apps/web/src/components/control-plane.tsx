...
function Detail({ detail, onUpdate }: { detail: TaskDetail; onUpdate: (task: TaskDetail) => void }) {
  ...
  async function action(name: 'approve' | 'reject' | 'cancel') {
  ...
  return (
    <section className="task-detail" aria-label="Task detail">
      ...
      <div className="detail-grid">
        <article className="panel"><h3>Plan & scan</h3><p>{text(metadata.framework, 'No framework detected')} · {text(metadata.packageManager, 'No package manager detected')}</p><p className="subtle">Verification: {text(metadata.testCommand, 'No detected test command')}</p>{detail.plans.length ? <p>{text(detail.plans[0].content)}</p> : <p className="subtle">No persisted Planner output.</p>}</article>
...
export function ControlPlane() {
  ...
  return <main className="shell">
    ...
    <div id="delivery">{detail ? <Detail detail={detail} onUpdate={(task) => { setDetail(task); void refresh(); }} /> : <section className="card"><h2>Agent run</h2><Empty>Select a task to inspect its plan, agents, verification, risk, approval, delivery, and evaluation records.</Empty></section>}</div>
  </main>;
}
