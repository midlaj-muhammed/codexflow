  ...
  return (
    <section className="task-detail" aria-label="Task detail">
      ...
      <div className="detail-grid">
        ...
        <article className="panel"><h3>Agent timeline</h3>{detail.agents.length ? <ol className="timeline">{detail.agents.map((agent) => <li key={String(agent.id)}><strong>{String(agent.role)}</strong><span>{String(agent.status)} · attempt {String(agent.attempt)}</span>{agent.error ? <small className="subtle">{String(agent.error)}</small> : null}</li>)}</ol> : <Empty>No persisted agent events yet.</Empty>}</article>
